import { button, div, h2, MainDOMSource, textarea, VNode } from '@cycle/dom'
...
function view(state$: Stream<State>): Stream<VNode> {
  return state$.map(({ text }) =>
    div([
      ...
      button({ attrs: { type: 'button' }, dataset: { action: 'speak' } }, [
        'Speak to Me!',
      ]),
      button({ attrs: { type: 'button' }, dataset: { action: 'navigate' } }, [
        'Page 1',
      ]),
    ]),
  )
